...
class Obstacle:
    ...
    def display(self):
        pygame.draw.rect(surface, YELLOW, (self.x_pos, self.y_pos, self.width, self.height))
        pygame.draw.rect(surface, YELLOW, (self.x_pos, self.y_pos + 650, self.width, self.height))
...
class Wall(Obstacle):
    ...
    def show(self):
        pygame.draw.rect(surface, GRAY, (self.x_pos, self.y_pos, self.width, self.height))
...
while running:
    ...
    if not death:
        ...
        def bullet_collision(): # sjekker om skuddet blir truffet
            for i in range(len(bullets)):
                if len(walls) > 0:
                    if collision(bullets[i], walls[0], 0, 3):
                        walls[0].lose_hp()
                        bullets.pop(i)
                        break
            for i in range(len(bullets)):
                for j in range(len(obstacles)):
                    if collision(bullets[i], obstacles[j], 0, 3) or collision(bullets[i], obstacles[j], 650, 3):
                        bullets.pop(i)
                        return
        ...
        def character_collision(): # sjekker om karakteren blir truffet slik at du dør
            global death
            if len(walls) > 0:
                if collision(character, walls[0], 0, 5):
                    death = True
                    return
            for i in range(len(obstacles)):
                if collision(character, obstacles[i], 0, 5) or collision(character, obstacles[i], 650, 5):
                    death = True
                    return
    ...
    for e in pygame.event.get():
        if e.type == pygame.KEYDOWN:
            ...
            elif e.key == pygame.K_RETURN and death:
                death = False
